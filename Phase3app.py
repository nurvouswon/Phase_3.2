...
st.title("🏆 MLB Home Run Predictor — AI-Powered Supercharged")
...
event_file = st.file_uploader("Event-Level CSV (required)", type=['csv', 'parquet'])
today_file = st.file_uploader("Today's CSV (required)", type=['csv', 'parquet'])

if event_file and today_file:
    event_df, today_df = safe_read(event_file), safe_read(today_file)
    event_df = event_df.dropna(axis=1, how='all')
    today_df = today_df.dropna(axis=1, how='all')

    if 'hr_outcome' not in event_df:
        st.error("'hr_outcome' column missing!")
        st.stop()

    mutual_feats = list(
        set(event_df.columns) &
        set(today_df.columns) - 
        {'hr_outcome', 'game_date', 'player_name', 'team', 'game_time'}
    )

    X = event_df[mutual_feats].fillna(-1)
    ...
    st.warning("Please upload required CSVs.")
